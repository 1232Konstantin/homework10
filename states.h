...
protected:
    ...
    void print_block(bool common=false)
    {
        ...
        if (!command_list->empty())
        {
            ...
            //используем доступ по мьютексу так-как экзкьютеры потенциально могут выполняться в разных потоках
            if  (common)
            {
                std::lock_guard<std::mutex> lock(m_executor.lock()->getMutex());
                help_func();
            }
            else help_func();
...
public:
    ...
    virtual void execute(std::string command, std::string time) final override
    {
        ...
        //запись в статический элемент экзекьютора только по мьютексу
        get_executor().lock()->getMutex().lock();
            auto list=get_executor().lock()->getCommonCommandList();
            list->push_back(std::make_pair(command,time));
        get_executor().lock()->getMutex().unlock();
